...
Queue.MaxPlayers = GetConvarInt("sv_maxclients", 400)
Queue.Debug = GetConvar("sv_debugqueue", "true") == "true" and true or false
Queue.DisplayQueue = GetConvar("sv_displayqueue", "false") == "true" and true or false
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local function done(msg, _deferrals)
        ...
        if not msg then
            deferrals.done()
            if Config.EnableGrace then if ids[1] ~= nil then Queue:AddPriority(ids[1], Config.GracePower, Config.GraceTime) end end
        else
...
if Config.DisableHardCap then
    Queue:DebugPrint("^1 [queue] Disabling hardcap ^7")
